...
function renderNavbar() {
  ...
  if (!navbar) return;
  navbar.innerHTML =
    '<li><a href="#" onclick="filterCategory(\'all\')" class="active">Todas</a></li>';
...
function filterCategory(category) {
  ...
  renderPizzas();
  toggleMenu(); // Fecha o menu após selecionar uma categoria
}
...
function deleteCategory(category) {
  if (
    confirm(
      `Tem certeza que deseja excluir a categoria "${category}"? Pizzas associadas ficarão sem categoria.`
    )
  ) {
...
function updateWhatsappNumber() {
  ...
  if (!/^\d+$/.test(whatsapp)) {
    alert(
      "O número do WhatsApp deve conter apenas dígitos (ex: 5511999999999)."
    );
    return;
  }
...
function addPizza() {
  ...
  const image =
    document.getElementById("pizza-image").value ||
    "https://images.unsplash.com/photo-1513104890138-7c749659a680?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80";
  ...
  pizzas.forEach((pizza) => {
    adminProducts.innerHTML += `
          <div class="product-admin-item">
              ...
              <span>${pizza.name} - ${formatPrice(pizza.price)} ${
      pizza.category ? `(${pizza.category})` : ""
    }</span>
              <button class="edit-btn" onclick="editPizza('${
                pizza.id
              }')">Editar</button>
...
function renderPizzas() {
  ...
  filteredPizzas.forEach((pizza) => {
    const pizzaCard = `
      ...
      pizza.name
    }" onclick="showDescription('${pizza.id}')">
              <h3>${pizza.name}</h3>
                ...
                (slice, i) =>
                  `<div class="flavor-item">${
                    slice
                      ? `Fatia ${
                          i + 1
                        } foi <span class="flavor-name">${slice}</span>`
                      : `Fatia ${i + 1} (vazia)`
                  }</div>`
              )
              .join("")
              ...
              .map(
                (slice, i) =>
                  `<div class="flavor-item">${
                    slice
                      ? `${
                          ["Primeiro", "Segundo", "Terceiro", "Quarto"][i]
                        } sabor foi <span class="flavor-name">${slice}</span>`
                      : `${
                          ["Primeiro", "Segundo", "Terceiro", "Quarto"][i]
                        } sabor (vazio)`
                  }</div>`
...
function renderPizzaSVG() {
  ...
  const size = window.innerWidth < 480 ? 200 : 300;
  svg.setAttribute("width", size);
  svg.setAttribute("height", size);
  svg.innerHTML = `<circle cx="${size / 2}" cy="${size / 2}" r="${
    size / 2 - 10
  ...
  for (let i = 0; i < slices; i++) {
    ...
    const nextAngle =
      (((i + 1) * (360 / slices) - 180 / slices) * Math.PI) / 180;
    ...
    slice.setAttribute(
      "d",
      `M ${size / 2} ${size / 2} L ${x1} ${y1} A ${size / 2 - 10} ${
        size / 2 - 10
      } 0 0 1 ${x2} ${y2} Z`
    );
    slice.setAttribute("fill", fillColor);
    slice.setAttribute("stroke", selectedSlice === i ? "#000" : "#fff");
...
function addCustomPizzaToCart() {
  const filledSlices = customPizzaSlices.filter(
    (slice) => slice !== null
  ).length;
  if (filledSlices === 0) {
...
function submitCheckout() {
  ...
  if (!pizzariaConfig.whatsapp) {
    alert(
      "O número do WhatsApp da pizzaria não foi configurado. Por favor, peça ao administrador para cadastrá-lo no painel administrativo."
    );
    return;
  }
    ...
    if (item.isCustom) {
      ...
      let flavorList =
        item.slices.length === 8
          ? item.slices
              .map((slice, i) =>
                slice ? `Fatia ${i + 1}: ${slice}` : `Fatia ${i + 1} (vazia)`
              )
              .join(", ")
          : item.slices
              .map(
                (slice, i) =>
                  `${["Primeiro", "Segundo", "Terceiro", "Quarto"][i]} sabor: ${
                    slice || "(vazio)"
                  }`
              )
              .join(", ");
      message += `Pizza montada (${
        item.quantity
      }x): ${flavorList} - ${formatPrice(customTotal)}\n`;
    } else {
      const pizza = pizzas.find((p) => p.id === item.pizzaId);
      if (pizza) {
        const itemTotal = pizza.price * item.quantity;
        total += itemTotal;
        message += `${pizza.name} (${item.quantity}x) - ${formatPrice(
          itemTotal
        )}\n`;
      }
    }
